  ...
  return (
    <section className="rounded-[1.75rem] border border-white/10 bg-panel/95 p-6 shadow-panel">
      <div className="flex items-center justify-between">
        <div>
          <p className="text-xs uppercase tracking-[0.28em] text-neutral-500">Macro Mood</p>
          <h2 className="mt-2 text-2xl font-semibold">Sentiment Overview</h2>
        </div>
        <p className="text-sm text-neutral-500">{summary.total_documents} analyzed documents</p>
      </div>
      ...
      <div className="mt-8 overflow-hidden rounded-[1.5rem] border border-white/10 bg-black/30">
        {cards.map((item) => (
          <div key={item.label} className="grid grid-cols-[100px_1fr_52px] items-center gap-3 px-5 py-3">
            ...
            <span className="text-right text-sm text-neutral-300">{item.value}</span>
          </div>
        ))}
      </div>
    </section>
